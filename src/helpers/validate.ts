...
export const geohash = (str: string, len: number): void => {
  if (str.length !== len)
    throw new Error(`expecting geohash to be at least ${len} characters`);
  for (const char of str) {
    if (!GEOHAS_CHARS.includes(char))
      throw new Error(`character '${char}' is not a valid geohash char`);
  }
};
  ...
  for (let i = 0; i < 40; i += 1) {
    ...
    if (
      (parseInt(addressHash[i], 16) > 7 &&
        addressNo0x[i].toUpperCase() !== addressNo0x[i]) ||
      (parseInt(addressHash[i], 16) <= 7 &&
        addressNo0x[i].toLowerCase() !== addressNo0x[i])
    ) {
      throw new Error(`invalid ethereum address: ${address}`);
    }
  }
...
export const zonePrice = (zonePrice: string): void => {
  if (Number(zonePrice) < 42)
    throw new Error(`Licence shop price is 42 DTH minimum`);
};
...
export const minStake = (amount: number) => {
  if (amount < constants.MIN_ZONE_STAKE) {
    throw new Error(
      `Staking amount must be superior at ${constants.MIN_ZONE_STAKE}`
    );
  }
};
...
export const tellerBuyerInfo = (isBuyer: boolean, buyRate: number) => {
  if (buyRate > 9999 || buyRate < -9999)
    throw new Error("buyRate minimum is -99.99, maximum is 99.99");
};

export const tellerSellerInfo = (isSeller: boolean, sellRate: number) => {
  if (!isSeller && sellRate)
    throw new Error("cannot set sellRate when not set as seller");
  if (sellRate > 9999 || sellRate < -9999)
    throw new Error("sellRate minimum is -99.99, maximum is 99.99");
};
...
export const telegramUsername = (messenger: string | undefined) => {
  if (typeof messenger !== "string")
    throw new Error("teller messenger should be a string");
  if (messenger.length < 1 || messenger.length > 16)
    throw new Error("telegram username should be between 1 and 16 characters");
  if (!/[A-Za-z0-9_]/g.test(messenger))
