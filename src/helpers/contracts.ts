...
export const getUniswapExchangeAddress = async (
...
export const get = async (
  ...
  if (!address) {
    // if no address passed in, load address from constants.js file
    let { name: networkName } = await provider.getNetwork();
    if (networkName === "unknown") networkName = "custom";
    address = CONTRACT_ADDRESSES[networkName][contractName]; // tslint:disable-line
    if (!address)
      throw new Error(
        `could not find contract address of ${contractName} on ${networkName}`
      );
  }
...
export const getErc20 = async (
  ...
  if (!address)
    throw new Error(
      `could not find token address of ${tokenName} on ${networkName}`
    );
