...
export const tellerArrToObj = (
  zoneAddr: string,
  tellerContractAddr: string,
  tellerArr: any[]
): ITeller => {
  ...
  return {
    ...
    referrer:
      tellerArr[7] !== constants.ADDRESS_ZERO ? tellerArr[7] : undefined,
    messenger:
      tellerArr[2] !== EMPTY_MESSENGER_FIELD
        ? convert.hexToAscii(tellerArr[2])
        : undefined,
...
export const getTeller = async (
  ...
  try {
    ...
    tellerInstance = await contract.get(
      provider,
  ...
  const teller = tellerArrToObj(
    zoneAddress,
    tellerAddress,
    await tellerInstance.getTeller()
  );
...
export const addTeller = async (
  ...
  const tellerSettings = settingsToBytes(
    tellerData.isBuyer,
    tellerData.isSeller
  );
  ...
  const tellerContract = await contract.get(
    wallet.provider,
    DetherContract.Teller,
    tellerAddress
  );
  ...
  return tellerContract
    ...
    .addTeller(
      ...
      tellerData.messenger
        ? util.stringToBytes(tellerData.messenger, 16)
        : EMPTY_MESSENGER_FIELD,
      tellerData.sellRate,
      tellerData.buyRate,
      tellerSettings,
      ...
      tellerData.description
        ? util.stringToBytes(tellerData.description, 32)
        : "",
      txOptions
    );
};
...
export const addComment = async (
  ...
  return tellerContract
    .connect(wallet)
    .addComment(util.ipfsHashToBytes32(ipfsHash), txOptions);
};
