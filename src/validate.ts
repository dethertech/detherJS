...
import { EXCHANGE_PAIRS } from './constants';

import { Unit, Token, IExchangePair } from './types';
...
export const exchangePair = (sellToken: Token, buyToken: Token) => {
  const acceptedPair = EXCHANGE_PAIRS.some((pair: IExchangePair) => {
    const [sell, buy] = pair.pair.split('-');
    return (sell === sellToken && buy === buyToken) ||
           (sell === buyToken  && buy === sellToken);
  });
  if (!acceptedPair) {
    throw new TypeError(`trading pair ${sellToken}-${buyToken} not implemented`);
  }
};
...
export const tellerMessenger = (messenger: string|undefined) => {
