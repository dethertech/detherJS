...
const addCountry = async (
  ...
  let txCount = 0;
  let error = false;
  console.log("add country pre for");
  for (
    let batchStartIdx = 0;
    ...
    // console.log('add country valuesBatch', valuesBatch, nonce)
    ...
    try {
      const receipt = await geoRegistryContract.updateLevel2batch(
        ...
        valuesBatch,
        { nonce, gasLimit: 7950000, gasPrice }
      );
      console.log("tx addCountry", receipt.hash);
      nonce += 1;
    } catch (e) {
      error = true;
  ...
  return { txCount, countryMap, newNonce, error };
};
